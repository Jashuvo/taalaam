...
async function checkAdmin(req: Request): Promise<Response | null> {
  ...
  if (email !== ADMIN_EMAIL) return new Response(JSON.stringify({ error: 'Forbidden' }), { status: 403, headers: { 'Content-Type': 'application/json', ...cors } });
  return null;
}
...
Deno.serve(async (req: Request) => {
  ...
  (async () => {
    try {
      ...
      const rawText = (await geminiGenerate(Deno.env.get('GEMINI_API_KEY')!, SYSTEM_PROMPT, userMessage))
        .trim().replace(/^```(?:json)?\n?/, '').replace(/\n?```$/, '').trim();
